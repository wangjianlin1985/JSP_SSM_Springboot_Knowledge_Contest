...
public class NoticeController extends BaseController {
	...
	public void list(String title,String noticeType,String publishDate,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (title == null) title = "";
		if (noticeType == null) noticeType = "";
		if (publishDate == null) publishDate = "";
		if(rows != 0)noticeService.setRows(rows);
		List<Notice> noticeList = noticeService.queryNotice(title, noticeType, publishDate, page);
	    /*计算总的页数和总的记录数*/
	    noticeService.queryTotalPageAndRecordNumber(title, noticeType, publishDate);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String title,String noticeType,String publishDate,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (title == null) title = "";
		if (noticeType == null) noticeType = "";
		if (publishDate == null) publishDate = "";
		List<Notice> noticeList = noticeService.queryNotice(title, noticeType, publishDate, currentPage);
	    /*计算总的页数和总的记录数*/
	    noticeService.queryTotalPageAndRecordNumber(title, noticeType, publishDate);
	    /*获取到总的页码数目*/
	    int totalPage = noticeService.getTotalPage();
	...
	public void OutToExcel(String title,String noticeType,String publishDate, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(title == null) title = "";
        if(noticeType == null) noticeType = "";
        if(publishDate == null) publishDate = "";
        List<Notice> noticeList = noticeService.queryNotice(title,noticeType,publishDate);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Notice信息记录"; 
        String[] headers = { "公告id","标题","新闻类别","发布时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<noticeList.size();i++) {
        	Notice notice = noticeList.get(i); 
        	dataset.add(new String[]{notice.getNoticeId() + "",notice.getTitle(),notice.getNoticeType(),notice.getPublishDate()});
        }
        /*
