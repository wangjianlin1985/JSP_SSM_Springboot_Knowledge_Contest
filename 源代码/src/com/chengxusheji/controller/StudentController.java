...
public class StudentController extends BaseController {
	...
	public void add(Student student, BindingResult br,
		...
		//这里比赛报名需要判断下业务逻辑,首先判断当前时间该比赛是否还可以报名
		Contest contest = contestService.getContest(student.getContest().getContestId());
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String nowTime = sdf.format(new java.util.Date());
		if(nowTime.compareTo(contest.getSignUpTime()) < 0 || nowTime.compareTo(contest.getEndTime()) > 0 ) {
			message = "现在时间不能报名哦！";
			writeJsonResponse(response, success, message);
			return ;
		}
		...
		int signUpNum = studentService.queryStudent("", contest, "", "", "").size();
		if(signUpNum == contest.getPersonNumber()) {
			message = "此比赛报名人数已满！";
			writeJsonResponse(response, success, message);
			return ;
		} 
		
		student.setSignUpTime(nowTime);
